...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  const connectWallet = async (walletType: "metamask" | "phantom") => {
    try {
      ...
      if (walletType === "metamask") {
        ...
        // connectMetaMask now automatically switches to Base Sepolia
        wallet = await connectMetaMask()
        
        // Verify we're on Base Sepolia
        if (wallet.network !== "Base Sepolia") {
          throw new Error("Please connect to Base Sepolia network to use this application")
        }
  ...
  const refreshWalletInfo = async () => {
    try {
      const currentWallet = await getCurrentWalletInfo()
      if (currentWallet) {
        // For MetaMask, verify we're on Base Sepolia
        if (currentWallet.walletType === "metamask" && currentWallet.network !== "Base Sepolia") {
          console.warn("Not connected to Base Sepolia network")
          // Don't disconnect, but alert the user
          alert("Please switch to Base Sepolia network to use all features of this application")
        }
